...
public class Evaluator extends HoodlumBaseVisitor<Double> {
    ...
    public Evaluator(Runtime rt){ this.rt = rt; }
    ...
    public Double visitOrderStmt(HoodlumParser.OrderStmtContext ctx) {
        ...
        if (isBuy) rt.buy(symbol, qty, price); else rt.sell(symbol, qty, price);
        return 0.0;
    }
    ...
    public Double visitIfStmtStmt(HoodlumParser.IfStmtStmtContext ctx) {
        ...
        if (cond != 0) visit(ctx.ifStmt().statement());
        return 0.0;
    }
    ...
    if (ctx.ID() != null) return rt.getVar(ctx.ID().getText());
    if (ctx.CASH() != null) return rt.getCash();
        if (ctx.getChildCount() == 3) {
            if ("(".equals(ctx.getChild(0).getText())) return visit(ctx.expr(0));
